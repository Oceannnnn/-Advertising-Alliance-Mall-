...
Page({
  ...
  collageList() {
    let token = app.globalData.token;
    util.http('goods/group_order', { id: this.data.good_id}, 'post',token).then(res => {
      if(res.code==200){
        this.setData({
  ...
  collect(e) {
    if (app.globalData.userInfo) {
      ...
      util.http('goods/collect_goods', { id: this.data.good_id}, 'post',token).then(res => {})
    } else {
      main.goLogin(1)
    }
  },
  ...
  disSelectValue(attrValueList, index, key, value) {
    ...
    this.setData({
      ...
      shopthumb: pro_info.original_img
    });
    for (var i = 0; i < attrValueList.length; i++) {
      if (attrValueList[i].selectedValue) {
  ...
  comfirm(e) {
    if (app.globalData.userInfo) {
      ...
      let priceId = this.data.includeGroup[0].price_id;
  ...
  comfirmBtn(e, priceId, id) {
    ...
    let jsonArr = [{ goods_id: id, priceId: priceId, count: num, store_id: store_id }];
